...
test('connects the existing Microsoft session to SharePoint notifications', () => {
  const html = fs.readFileSync(require.resolve('../portal.html'), 'utf8');
  assert.ok(html.includes('window.NexusNotifications.attachSharePoint(this.msal, acct)'));
  assert.ok(html.includes("if (nextView === 'connect' && window.NexusNotifications) window.NexusNotifications.refresh()"));
  assert.ok(!html.includes("fetch('notifications.json'"));
});
